package dev.necro.essentials.utils.pluginmanager;
...
public class PluginManagerUtils {
    ...
    private final NecroEssentials necroEssentials = NecroEssentials.getInstance();
    ...
    public boolean load(String pluginName) {
        ...
        if (!pluginFile.isFile()) {
            for (File file : Objects.requireNonNull(pluginDir.listFiles())) {
                if (file.toPath().endsWith(".jar")) {
                    try {
                        PluginDescriptionFile description = NecroEssentials.getInstance().getPluginLoader().getPluginDescription(file);
        ...
        if (necroEssentials.getVersionSupport().getCommandWrap().isUsed()) {
            Bukkit.getScheduler().runTaskLater(NecroEssentials.getInstance(), () -> {
                ...
                for (Map.Entry<String, Command> entry : knownCommands.entrySet().stream().filter(stringCommandEntry -> stringCommandEntry.getValue() instanceof PluginIdentifiableCommand).filter(stringCommandEntry -> {
                    ...
                    necroEssentials.getVersionSupport().getCommandWrap().wrap(command, alias);
                }

                if (Bukkit.getOnlinePlayers().size() >= 1) {
                    Bukkit.getOnlinePlayers().forEach(player -> necroEssentials.getVersionSupport().reloadCommands(player));
                }
            }, 10L);
        }
    ...
    public boolean unload(Plugin plugin) {
        if (necroEssentials.getVersionSupport().getCommandWrap().isUsed()) {
            ...
            for (Map.Entry<String, Command> entry : knownCommands.entrySet().stream().filter(stringCommandEntry -> stringCommandEntry.getValue() instanceof PluginIdentifiableCommand).filter(stringCommandEntry -> {
                ...
                String alias = entry.getKey();
                necroEssentials.getVersionSupport().getCommandWrap().unwrap(alias);
            }

            if (Bukkit.getOnlinePlayers().size() >= 1) {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    necroEssentials.getVersionSupport().reloadCommands(player);
                }
            }
        }
