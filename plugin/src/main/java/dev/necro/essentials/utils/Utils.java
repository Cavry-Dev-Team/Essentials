package dev.necro.essentials.utils;

import com.google.common.base.Joiner;
import dev.necro.essentials.NecroEssentials;
import dev.necro.essentials.callbacks.IsDoubleCallback;
...
public class Utils {
    ...
    static {
        plugin = NecroEssentials.getInstance();
    ...
    public String getPluginDescription() {
        if (pluginDescription == null) {
            pluginDescription = plugin.getMainConfigManager().getPrefix() + "§eThis server is running §bNecroEssentials §bv" + plugin.getDescription().getVersion() + " §eby §b" + Joiner.on(",").join(plugin.getDescription().getAuthors());
        }
    ...
    public boolean checkPermission(CommandSender target, String permission, boolean others, boolean showPermission, boolean silent, @Nullable String command) {
        permission = "necroessentials." + permission.toLowerCase();
        if (others) {
            permission += ".others";
        }
    ...
    public void applyMetadata(Player player, String metadata, Object value) {
        player.setMetadata(metadata, new FixedMetadataValue(NecroEssentials.getInstance(), value));
    }
    ...
    public void removeMetadata(Player player, String metadata) {
        player.removeMetadata(metadata, NecroEssentials.getInstance());
    }
