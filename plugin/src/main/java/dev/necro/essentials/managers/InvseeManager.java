...
import dev.necro.essentials.utils.ItemBuilder;
...
public class InvseeManager {
    ...
    public InvseeManager(NecroEssentials plugin) {
        this.plugin = plugin;
        ...
        if (plugin.getMainConfigManager().isOldInvsee()) {
            ...
            this.separatorItem = NecroEssentials.getInstance().getVersionSupport().getItemByName("blackglasspane", 1, 0);
            this.infoItem = NecroEssentials.getInstance().getVersionSupport().getItemByName("paper", 1, 0);
            this.locationItem = NecroEssentials.getInstance().getVersionSupport().getItemByName("compass", 1, 0);
            this.effectsItem = NecroEssentials.getInstance().getVersionSupport().getItemByName("brewing_stand_item", 1, 0);
    ...
    public void invsee(Player player, Player target) {
        ...
        inventory.setItem(51, new ItemBuilder(locationItem.getType())
                .setName("§b" + target.getName() + "§e's location:")
        ...
        inventory.setItem(52, new ItemBuilder(infoItem.getType())
                .setName("§b" + target.getName() + "§e's info:")
        ...
        ItemBuilder activeEffects = new ItemBuilder(effectsItem.getType()).setName("§b" + target.getName() + "§e's active potion effects:");
    ...
    public void refresh(Player player, Player target, Inventory topInventory) {
        Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                ...
                ItemBuilder activeEffects = new ItemBuilder(this.effectsItem.getType()).setName("§b" + target.getName() + "§e's active potion effects:");
                List<String> effects = this.getActiveEffects(target);
                if (!effects.isEmpty()) {
                    effects.forEach(activeEffects::addLoreLine);
                }
