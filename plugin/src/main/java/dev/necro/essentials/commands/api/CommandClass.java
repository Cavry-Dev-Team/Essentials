package dev.necro.essentials.commands.api;

import cloud.commandframework.annotations.suggestions.Suggestions;
import cloud.commandframework.context.CommandContext;
import dev.necro.essentials.NecroEssentials;
import dev.necro.essentials.callbacks.CanSkipCallback;
...
public abstract class CommandClass {
    ...
    protected NecroEssentials plugin = NecroEssentials.getInstance();
    ...
    public CanSkipCallback canSkip(String action, TargetsCallback targetsCallback, CommandSender sender) {
        ...
        if (targetsCallback.size() >= 75) {
            return new CanSkipCallback(sender, false, Collections.singletonList(
                    plugin.getMainConfigManager().getPrefix() + "§eAre you sure you want to execute §b" + action + " §eon §b" + targetsCallback.size() + " §eplayers?"
            ));
        }
