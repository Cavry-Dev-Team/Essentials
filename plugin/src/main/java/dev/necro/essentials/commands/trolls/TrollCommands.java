...
public class TrollCommands extends CommandClass {
    ...
    @ProxiedBy("fakebreak")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt fakebreak [target] [toggle]")
    ...
    @ProxiedBy("fakeplace")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt fakeplace [target] [toggle]")
    ...
    @ProxiedBy("lagback")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt lagback [target] [toggle]")
    ...
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt nointeract [target] [toggle]")
    ...
    @ProxiedBy("nopickup")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt nopickup [target] [toggle]")
    ...
    @ProxiedBy("onetap")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt onetap [target] [toggle]")
    ...
    @ProxiedBy("sticky")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt sticky [target] [toggle]")
    ...
    @ProxiedBy("nodamage")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt nodamage [target] [toggle]")
    ...
    @ProxiedBy("nohit")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt nohit [target] [toggle]")
    ...
    @ProxiedBy("noplace")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt noplace [target] [toggle]")
    ...
    @ProxiedBy("nobreak")
    @CommandMethod("necrotrolls|necrotroll|trolls|troll|lt nobreak [target] [toggle]")
    ...
    private void toggleTroll(CommandSender sender, String targetName, String toggle, TrollType trollType) {
        ...
        if (targets.size() > 1) {
            sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eToggled §6" + trollType.getDisplay() + " §efor §b" + targets.size() + " §eplayers.");
        } else if (targets.size() == 1 || (!(sender instanceof Player) || targets.doesNotContain((Player) sender))) {
            targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eToggled §6" + trollType.getDisplay() + " §efor §b" + target.getName() + "§e: " + Utils.colorizeTrueFalse(target.hasMetadata(trollType.getMetadataKey()), TrueFalseType.ON_OFF)));
        }
    }
