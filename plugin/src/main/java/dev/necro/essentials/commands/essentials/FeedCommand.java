...
public class FeedCommand extends CommandClass {
    ...
    public void feedCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            ...
            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eFed §b" + target.getName() + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eFed §b" + targets.size() + " §eplayers.");
                }
