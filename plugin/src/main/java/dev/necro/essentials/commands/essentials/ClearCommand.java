...
public class ClearCommand extends CommandClass {
    ...
    public void clearCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            targets.forEach(target -> {
                switch (inventoryScope) {
                    ...
                    case SPECIFIC: {
                        ...
                        if (silent == null || !silent) {
                            target.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eCleared all §b" + inventoryScope.getItemStack().getType() + " §efrom your inventory.");
                        }
                        break;
                    }
                }
            ...
            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eCleared §6" + inventoryScope.getDisplay() + " §efor §b" + target.getName() + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eCleared §6" + inventoryScope.getDisplay() + " §efor §b" + targets.size() + " §eplayers.");
                }
