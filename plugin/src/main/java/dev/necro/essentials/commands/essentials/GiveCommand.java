...
public class GiveCommand extends CommandClass {
    ...
    public void giveCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            targets.forEach(target -> {
                ...
                target.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eYou have been given §b" + item.getAmount() + "x " + item.getType().toString() + "§e.");
            });

            if (targets.size() > 1) {
                sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eGiven §6" + item.getAmount() + "x " + item.getType().toString() + " §eto §b" + targets.size() + " §eplayers.");
            } else if ((!(sender instanceof Player)) || (targets.doesNotContain((Player) sender) && !targetName.equals("self"))) {
                targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eGiven §6" + item.getAmount() + "x " + item.getType().toString() + " §eto §b" + target.getName() + "§e."));
            }
    ...
    public void iCommand(
        ...
        sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eYou have been given §b" + item.getAmount() + "x " + item.getType().toString() + "§e.");
    }
}
