...
import dev.necro.essentials.enums.InventoryScope;
...
public class FixCommand extends CommandClass {
    ...
    public void fixCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            targets.forEach(target -> {
                switch (inventoryScope) {
                    ...
                    case SPECIFIC: {
                        ...
                        if (silent == null || !silent) {
                            target.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eRepaired all §b" + inventoryScope.getItemStack().getType() + " §ein your inventory.");
                        }
                        break;
                    }
                }
            ...
            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eRepaired §6" + inventoryScope.getDisplay() + " §efor §b" + target.getName() + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eRepaired §6" + inventoryScope.getDisplay() + " §efor §b" + targets.size() + " §eplayers.");
                }
