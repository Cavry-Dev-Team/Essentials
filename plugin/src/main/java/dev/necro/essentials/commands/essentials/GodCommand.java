...
public class GodCommand extends CommandClass {
    ...
    public void godCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            ...
            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eGod mode for §b" + target.getName() + "§e: " + Utils.colorizeTrueFalse(target.hasMetadata("GOD"), TrueFalseType.ON_OFF) + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eToggled god for §b" + targets.size() + " §eplayers.");
                }
