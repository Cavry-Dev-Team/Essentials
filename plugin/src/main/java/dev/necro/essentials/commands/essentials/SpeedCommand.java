...
import dev.necro.essentials.enums.SpeedType;
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            targets.forEach(target -> {
                ...
                if (silent == null || !silent) {
                    target.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eYour §b" + speedType.getDisplay() + " §espeed has been set to §b" + speedType.getSpeed() + "§e.");
                }
            });

            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eSet §6" + speedType.getDisplay() + " §espeed to §b" + speedType.getSpeed() + " §efor §b" + target.getName() + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eSet §6" + speedType.getDisplay() + " §espeed to §b" + speedType.getSpeed() + " §espeed for §b" + targets.size() + " §eplayers.");
                }
