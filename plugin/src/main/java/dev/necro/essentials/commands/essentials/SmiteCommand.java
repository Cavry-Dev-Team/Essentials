...
public class SmiteCommand extends CommandClass {
    ...
    public void smiteCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
                    ...
                    if (others) {
                        sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eSmitten §b" + finalTargets.size() + " §eplayers.");
                    } else if ((!(sender instanceof Player)) || (finalTargets.doesNotContain((Player) sender) && !targetName.equals("self"))) {
                        finalTargets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eSmitten §b" + target.getName() + "§e."));
                    }
