...
public class TopCommand extends CommandClass {
    ...
    public void topCommand(
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            ...
            if (others) {
                if (targets.size() == 1) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eTeleported §b" + target.getName() + "§e."));
                } else {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eTeleported §b" + targets.size() + " §eplayers.");
                }
