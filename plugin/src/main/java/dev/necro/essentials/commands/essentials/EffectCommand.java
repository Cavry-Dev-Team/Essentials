    ...
    public void effectCommand(
        ...
        if (effectName.equalsIgnoreCase("clear")) {
            plugin.getConfirmationManager().requestConfirmation(() -> {
                ...
                if (others) {
                    sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eRemoved all potion effects from §b" + targets.size() + " §eplayers.");
                } else if ((!(sender instanceof Player)) || (targets.doesNotContain((Player) sender) && !targetName.equals("self"))) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eRemoved all potion effects from §b" + target.getName() + "§e."));
                }
        ...
        plugin.getConfirmationManager().requestConfirmation(() -> {
            ...
            if (potionEffectList.size() == 1) {
                ...
                if (others) {
                    if (targets.size() == 1) {
                        targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied §6" + potionEffect.getType().getName() + ":" + finalAmplifier + " §efor §b" + durationSeconds + " seconds §eto §b" + target.getName() + "§e."));
                    } else {
                        sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied §6" + potionEffect.getType().getName() + ":" + finalAmplifier + " §efor §b" + durationSeconds + " seconds §eto §b" + targets.size() + " §eplayers.");
                    }
                } else if ((!(sender instanceof Player)) || (targets.doesNotContain((Player) sender) && !targetName.equals("self"))) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied §6" + potionEffect.getType().getName() + ":" + finalAmplifier + " §efor §b" + durationSeconds + " seconds §eto §b" + target.getName() + "§e."));
                }
                ...
                if (others) {
                    if (targets.size() == 1) {
                        targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied all potion effects to §b" + target.getName() + "§e."));
                    } else {
                        sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied all potion effects to §b" + targets.size() + " §eplayers.");
                    }
                } else if ((!(sender instanceof Player)) || (targets.doesNotContain((Player) sender) && !targetName.equals("self"))) {
                    targets.stream().findFirst().ifPresent(target -> sender.sendMessage(plugin.getMainConfigManager().getPrefix() + "§eApplied all potion effects to §b" + target.getName() + "§e."));
                }
